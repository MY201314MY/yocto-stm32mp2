...
static const struct ili9881c_instr kd050hdfia020_init[] = {
	ILI9881C_SWITCH_PAGE_INSTR(3),
	ILI9881C_COMMAND_INSTR(0x01, 0x01),
	ILI9881C_COMMAND_INSTR(0x02, 0x02),
...
static int ili9881c_prepare(struct drm_panel *panel)
{
	...
	dev_info(&ctx->dsi->dev, "read panel id...");
	if(mipi_dsi_dcs_read(ctx->dsi, 0xDA, rxbuffer, 1))
	{
		dev_info(&ctx->dsi->dev, "ID1 : 0x%02X", rxbuffer[0]);
	}
	if(mipi_dsi_dcs_read(ctx->dsi, 0xDB, rxbuffer, 1))
	{
		dev_info(&ctx->dsi->dev, "ID2 : 0x%02X", rxbuffer[0]);
	}
	if(mipi_dsi_dcs_read(ctx->dsi, 0xDC, rxbuffer, 1))
	{
		dev_info(&ctx->dsi->dev, "ID3 : 0x%02X", rxbuffer[0]);
	}
	...
	ret = ili9881c_send_cmd_data(ctx, 0x3A, 0x70);
	if (ret)
		return ret;
